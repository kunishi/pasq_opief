		...
		function start() { //ここからメイン
			...
			for (i = 0; i < cn; i++) { //コンテンツの個数分ループ
				...
				if (c_length < 0.0005) { //コンテンツと表示パノラマの距離がある程度小さければ
					c_height = ccd.Contents.floor.Content[i].coords.@height; //近いコンテンツの情報を格納していく
					c_radius = ccd.Contents.floor.Content[i].range.@radius; //コンテンツは複数同時表示もある
					c_name = ccd.Contents.floor.Content[i].detail.@name; //現在は単一のコンテンツのみ
					c_id = ccd.Contents.floor.Content[i].@contentid; //次期対応しなければ…
			...
			for (i = 0; i < cn; i++) {
				text_field.appendText(ccd.Contents.floor.Content[i].detail.@name + "\n");
				//strTag += "<p><a href='event:i'><font color='#0000FF'><b>ccd.Contents.floor.Content[i].detail.@name</b></font></a></p>"
			}
			...
			function comHandler(e: Event) {
				...
				fullon.y = stage.stageHeight - 100;
				fulloff.x = stage.stageWidth - 250;
				fulloff.y = stage.stageHeight - 100;
				...
				for (i = 0; i < ccd_len1; i++) {
					...
					var cnt_floor: String = "../" + "../" + cnt_name + "/" + ccd.Contents.floor.Content[i].@contentid + "/PasQk.swf";
					ary[i] = {
						label: cnt_name,
						data: cnt_floor
					};
				...
				myTextArea.text = "現在:" + n_floor + "　利用形式:" + floor_cont_name;
				myTextArea.setSize(230, 24);
				myTextArea.move(15, 360);
				addChild(myTextArea);
				...
				//c_data.addEventListener(MouseEvent.CLICK,trace_c);
			...
			function cont_select(event: MouseEvent): void {
				...
				panoid = xml.Panoramas.Panorama[i].@panoid.split("pano").join(""); //新しいパノラマ画像の情報を取得
				...
				function texture2(event: Event) { //表示しているパノラマ画像の更新
					for (i = 0; i < cn; i++) {
						c_id = ccd.Contents.floor.Content[i].@contentid;
...
/*					map.setCenter(new LatLng(lat, lng));
					...
					for (i = 0; i < cn; i++) { //新しいコンテンツの検索（先述のものと同様）
						...
						if (c_length < 0.00021) {
							c_height = ccd.Contents.floor.Content[i].coords.@height;
							c_radius = ccd.Contents.floor.Content[i].range.@radius;
							c_name = ccd.Contents.floor.Content[i].detail.@name;
							c_id = ccd.Contents.floor.Content[i].@contentid;
							//c_data = new Text3D(c_id, new HelveticaBold() , letterformat);
							c_data.text = c_id;
			...
			function onMapReady(event: Event): void {
				//map.setCenter(new LatLng(lat, lng), 19, MapType.NORMAL_MAP_TYPE);
				//map.enableScrollWheelZoom();
				//map.enableContinuousZoom()

				//var cmap: Loader = new Loader();
				//cmap.load(new URLRequest(usebmd)); // Read it!
				...
				//		cmap,
				//		new LatLngBounds(new LatLng(34.668165, 133.916167), new LatLng(34.668355, 133.91665)));
			...
			function addMarkers(event: Event): void { //マーカー設定
				...
				//for (i = 0; i < cn; i++) {
				//	markerOptions.tooltip = ccd.Contents.floor.Content[i].detail.@name;
